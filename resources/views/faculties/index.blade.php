<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">
            {{ __('Manajemen Fakultas / Direktorat') }}
        </h2>
    </x-slot>
    ...
    <div x-data="crud">
                            ...
                            <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                                <thead
                                    ...
                                    <tr>
                                        ...
                                        <th scope="col" class="py-3 px-6">Nama Fakultas / Direktorat</th>
                                ...
                                <tbody>
                                    @forelse ($faculties as $index => $faculty)
                                        <tr
                                            ...
                                            <td class="py-4 px-6">{{ $faculty->name }}</td>
                                            <td class="py-4 px-6">{{ $faculty->code }}</td>
                                            <td class="py-4 px-6 flex space-x-2">
                                                <button
                                                    @click="showModal = true; isEditMode = true; form.name = '{{ $faculty->name }}'; form.code = '{{ $faculty->code }}'; form.id = {{ $faculty->id }}"
                                    ...
                                    @empty
                                        <tr>
                                            <td colspan="4" class="py-4 px-6 text-center">Tidak ada data fakultas.
                                            </td>
                                        </tr>
                                    @endforelse
                                </tbody>
        ...
        <div x-show="showModal" x-cloak
            ...
            <div class="bg-white dark:bg-gray-800 p-8 rounded-lg shadow-xl w-full max-w-md"
                ...
                <h2 class="text-2xl font-bold mb-6" x-text="isEditMode ? 'Edit Fakultas' : 'Tambah Fakultas Baru'"></h2>
                <form :action="isEditMode ? '/faculties/' + form.id : '{{ route('faculties.store') }}'" method="POST">
                    @csrf
        ...
        <div x-show="showImportModal" x-cloak
            ...
            <div class="bg-white dark:bg-gray-800 p-8 rounded-lg shadow-xl w-full max-w-md"
                ...
                <h2 class="text-2xl font-bold mb-6">Impor Data Fakultas</h2>
                <form action="{{ route('faculties.import') }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="mb-4">
                        ...
                        <p class="mt-2 text-xs text-gray-500 dark:text-gray-400">Pastikan file Excel memiliki header:
                            <strong>nama_fakultas</strong> dan <strong>kode</strong>.
                        </p>
                    </div>
                    <div class="flex justify-end space-x-4">
