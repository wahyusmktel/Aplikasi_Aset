                                ...
                                <tr class="hover:bg-gray-50/50 dark:hover:bg-gray-800/40 transition-all duration-200 group">
                                    ...
                                    <td class="px-6 py-4 text-center">
                                        <div class="flex justify-center space-x-2">
                                            ...
                                            @endif
                                            <button @click="showModal = true; isEditMode = true; form.id = {{ $employee->id }}; form.name = '{{ addslashes($employee->name) }}'; form.nip = '{{ addslashes($employee->nip) }}'; form.position = '{{ addslashes($employee->position) }}'; form.is_sarpra_it_lab = {{ $employee->is_sarpra_it_lab ? 'true' : 'false' }}; form.is_headmaster = {{ $employee->is_headmaster ? 'true' : 'false' }}; form.is_kaur_it = {{ $employee->is_kaur_it ? 'true' : 'false' }}"
            ...
            <div class="bg-white/90 dark:bg-gray-900/90 backdrop-blur-xl p-8 rounded-3xl shadow-2xl w-full max-w-md border border-white/20"
                ...
                <form :action="isEditMode ? '/employees/' + form.id : '{{ route('employees.store') }}'" method="POST" class="space-y-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div class="flex items-center">
                            <input type="hidden" name="is_kaur_it" value="0">
                            <input type="checkbox" name="is_kaur_it" id="is_kaur_it" x-model="form.is_kaur_it" value="1"
                                class="w-5 h-5 rounded border-gray-300 text-primary-600 focus:ring-primary-500 transition-all">
                            <label for="is_kaur_it" class="ml-3 text-sm font-bold text-gray-700 dark:text-gray-300">
                                Kaur IT
                            </label>
                        </div>
                    </div>
