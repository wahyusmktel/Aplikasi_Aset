<x-app-layout>
    ...
    <style>
        ...
        #selectionBar { transition: transform .2s ease, opacity .2s ease; }
            ...
            <div class="relative rounded-2xl bg-gradient-to-r from-indigo-600 via-violet-600 to-purple-700 shadow-lg overflow-hidden p-7">
                ...
                <div class="relative z-10 flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
                    <div class="text-white">
                        <p class="text-indigo-200 text-sm font-semibold uppercase tracking-widest mb-1">Sistem Tanda Tangan Digital — SARPRA</p>
                ...
                <div class="xl:col-span-1 space-y-5">
                    ...
                    <div class="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
                        ...
                        <div class="p-6 space-y-3">
                            ...
                            <div class="flex justify-between items-center text-sm">
                                <span class="text-gray-500 font-medium">Dokumen BAST Ditandatangani</span>
                    ...
                    <div class="bg-white rounded-2xl border border-blue-200 shadow-sm overflow-hidden">
                        <div class="px-6 py-4 border-b border-blue-100 flex items-center gap-3">
                            ...
                            <div>
                                <h4 class="font-bold text-gray-800 text-sm">Import dari SISFO</h4>
                                <p class="text-xs text-gray-400">Gunakan tanda tangan yang sudah ada di sisfo.smktelkom-lpg.id</p>
                            </div>
                        </div>
                        <div class="p-6">
                            <form action="{{ route('tanda-tangan.import-sisfo') }}" method="POST" class="space-y-3">
                                @csrf
                                <div>
                                    <label class="block text-xs font-bold text-gray-600 uppercase tracking-wider mb-1.5">Email SISFO</label>
                                    <input type="email" name="sisfo_email" placeholder="email@smktelkom-lpg.id"
                                        class="w-full rounded-xl border-gray-200 text-sm focus:ring-blue-500 focus:border-blue-500">
                                </div>
                                <div>
                                    <label class="block text-xs font-bold text-gray-600 uppercase tracking-wider mb-1.5">Password SISFO</label>
                                    <input type="password" name="sisfo_password" placeholder="••••••••"
                                        class="w-full rounded-xl border-gray-200 text-sm focus:ring-blue-500 focus:border-blue-500">
                                </div>
                                <button type="submit"
                                    class="w-full py-2.5 px-4 bg-blue-600 hover:bg-blue-700 text-white font-bold rounded-xl text-sm transition-all shadow-sm">
                                    Import Gambar TTD dari SISFO
                                </button>
                            </form>
                        </div>
                    </div>
                    ...
                    <div class="bg-gradient-to-br from-slate-800 to-slate-900 rounded-2xl p-5 text-white">
                        ...
                        <ul class="space-y-2 text-sm text-slate-300">
                            <li class="flex items-start gap-2">
                                ...
                                Hash SHA-256 untuk setiap dokumen BAST
                            </li>
                            <li class="flex items-start gap-2">
                                <svg class="w-4 h-4 text-green-400 flex-shrink-0 mt-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z"/></svg>
                ...
                <div class="xl:col-span-2">
                    <div class="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
                        ...
                        <div class="px-6 py-4 border-b border-gray-100 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                            <div>
                                <h4 class="font-bold text-gray-800">Riwayat BAST Ditandatangani</h4>
                                                ...
                                                <div class="flex items-start justify-between gap-3">
                                                    <div class="min-w-0">
                                                        ...
                                                        <p class="text-xs text-gray-400 mt-0.5">
                                                            ...
                                                            &nbsp;·&nbsp;
                                                            {{ \Carbon\Carbon::parse($doc->signed_at)->translatedFormat('d F Y, H:i') }} WIB
                                                        </p>
                                                    ...
                                                    <div class="flex items-center gap-2 flex-shrink-0">
                                                        @if($doc->is_valid)
                                                            ...
                                                            <a href="{{ route('tanda-tangan.verify', $doc->token) }}" target="_blank"
                        ...
                        @else
                            <div class="py-16 text-center">
                                <div class="flex flex-col items-center gap-3 text-gray-400">
                                    ...
                                    <p class="font-bold text-base text-gray-500">Belum ada dokumen BAST ditandatangani</p>
                                    <p class="text-sm text-gray-400">Tanda tangan digital akan muncul di sini setelah Anda menandatangani dokumen BAST.</p>
                                </div>
                            </div>
                        @endif
                    </div>
    ...
    <div id="selectionBar" class="fixed bottom-6 left-1/2 -translate-x-1/2 z-40 bar-hidden">
        <div class="flex items-center gap-3 bg-gray-900 text-white px-5 py-3 rounded-2xl shadow-2xl border border-gray-700">
            ...
            <button onclick="clearSelection()" class="text-gray-400 hover:text-white text-xs font-semibold transition">Batalkan</button>
        </div>
    </div>
    ...
    <div id="bulkRevokeModal" class="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 hidden items-center justify-center p-4">
        <div class="bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden">
            <div class="px-6 py-5 bg-gradient-to-r from-red-600 to-rose-700">
            ...
            <div class="p-6">
                ...
                <form id="bulkRevokeForm" method="POST" class="space-y-4">
                    ...
                    <div>
                        ...
                        <input type="text" name="revoke_reason" id="bulkRevokeReason" placeholder="cth: Dokumen direvisi..."
                            class="w-full rounded-xl border-gray-200 text-sm focus:ring-red-500 focus:border-red-500">
                    </div>
                    <div>
                        <label class="block text-xs font-bold text-gray-600 uppercase tracking-wider mb-1.5">PIN Keamanan</label>
    ...
    <script>
        ...
        document.getElementById('revokeModal').addEventListener('click', e => { if (e.target === e.currentTarget) closeRevokeModal(); });

        const checkAll     = document.getElementById('checkAll');
        const selectionBar = document.getElementById('selectionBar');
        const selectionCount = document.getElementById('selectionCount');

        function getChecked() { return [...document.querySelectorAll('.doc-checkbox:checked')].map(cb => cb.value); }
        ...
        document.querySelectorAll('.doc-checkbox').forEach(cb => cb.addEventListener('change', updateSelectionBar));
        if (checkAll) {
        ...
        function openBulkRevokeModal(mode) {
            ...
            if (mode === 'all') {
                ...
                infoEl.innerHTML = '<p class="font-bold mb-1">⚠️ Tindakan ini akan mencabut <strong>semua {{ $validCount }} dokumen aktif</strong> milik Anda.</p>';
            } else {
                ...
                infoEl.innerHTML = '<p class="font-bold mb-1">Anda akan mencabut <strong>' + tokens.length + ' dokumen</strong> yang dipilih.</p>';
                tokens.forEach(token => {
                    const input = document.createElement('input');
                    input.type = 'hidden'; input.name = 'tokens[]'; input.value = token;
        ...
        document.getElementById('bulkRevokeModal').addEventListener('click', e => { if (e.target === e.currentTarget) closeBulkRevokeModal(); });
    </script>
    @endpush
</x-app-layout>
