    ...
    <div x-data="crud">
                            ...
                            <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                                <thead
                                    ...
                                    <tr>
                                        ...
                                        <th scope="col" class="py-3 px-6">Nama Lembaga</th>
                                ...
                                <tbody>
                                    @forelse ($institutions as $index => $institution)
                                        <tr
                                            ...
                                            <td class="py-4 px-6">{{ $institution->name }}</td>
                                            <td class="py-4 px-6">{{ $institution->code }}</td>
                                            <td class="py-4 px-6 flex space-x-2">
                                                <button
                                                    @click="showModal = true; isEditMode = true; form.name = '{{ $institution->name }}'; form.code = '{{ $institution->code }}'; form.id = {{ $institution->id }}"
                                    ...
                                    @empty
                                        <tr>
                                            <td colspan="4" class="py-4 px-6 text-center">Tidak ada data lembaga.
                                            </td>
                                        </tr>
                                    @endforelse
                                </tbody>
        ...
        <div x-show="showModal" x-cloak
            ...
            <div class="bg-white dark:bg-gray-800 p-8 rounded-lg shadow-xl w-full max-w-md"
                ...
                <h2 class="text-2xl font-bold mb-6" x-text="isEditMode ? 'Edit Lembaga' : 'Tambah Lembaga Baru'"></h2>
                <form :action="isEditMode ? '/institutions/' + form.id : '{{ route('institutions.store') }}'"
                    method="POST">
                    @csrf
        ...
        <div x-show="showImportModal" x-cloak
            ...
            <div class="bg-white dark:bg-gray-800 p-8 rounded-lg shadow-xl w-full max-w-md"
                ...
                <h2 class="text-2xl font-bold mb-6">Impor Data Lembaga</h2>
                <form action="{{ route('institutions.import') }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    <div class="mb-4">
                        ...
                        <p class="mt-2 text-xs text-gray-500 dark:text-gray-400">Pastikan file Excel memiliki header:
                            <strong>nama_lembaga</strong> dan <strong>kode</strong>.</p>
                    </div>
                    <div class="flex justify-end space-x-4">
