<x-app-layout>
    <div x-data="crud" class="space-y-8">
        ...
        <div class="relative overflow-hidden rounded-3xl bg-gradient-to-r from-primary-600 to-primary-800 p-8 shadow-2xl animate-fadeIn">
            <div class="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    <h2 class="text-3xl font-bold text-white mb-2">{{ __('Manajemen Ruangan') }}</h2>
                    <p class="text-primary-100 opacity-90">Kelola lokasi dan denah ruangan untuk penempatan aset.</p>
                </div>
            ...
            <div class="p-8">
                ...
                <div class="flex flex-col md:flex-row justify-between items-center mb-8 gap-4">
                    <div class="w-full md:w-96">
                        <form action="{{ route('rooms.index') }}" method="GET" class="relative group">
                            <input type="text" name="search" placeholder="Cari ruangan..."
                    ...
                    <table class="w-full text-left border-collapse">
                        <thead>
                            <tr class="bg-gray-50 dark:bg-gray-800/50">
                                ...
                                <th class="px-6 py-4 text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider">Nama Ruangan</th>
                                ...
                                <tr class="hover:bg-gray-50/50 dark:hover:bg-gray-800/40 transition-all duration-200 group">
                                    ...
                                    <td class="px-6 py-4">
                                        <div class="flex items-center">
                                            ...
                                            <span class="text-sm font-semibold text-gray-800 dark:text-gray-200">{{ $room->name }}</span>
                                        </div>
                                    </td>
                                    <td class="px-6 py-4">
                                    ...
                                    <td class="px-6 py-4 text-center">
                                        <div class="flex justify-center space-x-2">
                                            <button @click="showModal = true; isEditMode = true; form.name = '{{ $room->name }}'; form.id = {{ $room->id }}"
                                ...
                                <tr>
                                    <td colspan="4" class="px-6 py-12 text-center">
                                        <div class="flex flex-col items-center">
                                            ...
                                            <p class="text-gray-500 dark:text-gray-400 font-medium">Tidak ada data ruangan ditemukan.</p>
                                        </div>
                                    </td>
                                </tr>
                            @endforelse
        ...
        <div x-show="showModal" 
            ...
            <div class="bg-white/90 dark:bg-gray-900/90 backdrop-blur-xl p-8 rounded-3xl shadow-2xl w-full max-w-md border border-white/20"
                ...
                <div class="flex justify-between items-center mb-6">
                    <h2 class="text-2xl font-bold text-gray-800 dark:text-white" x-text="isEditMode ? 'Edit Ruangan' : 'Tambah Ruangan'"></h2>
                    <button @click="showModal = false" class="text-gray-400 hover:text-gray-600 transition-colors">
                        <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12" /></svg>
                    </button>
                </div>
                
                <form :action="isEditMode ? '/rooms/' + form.id : '{{ route('rooms.store') }}'" method="POST" class="space-y-6">
                    @csrf
                    ...
                    <div>
                        <label class="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-2">Nama Ruangan</label>
        ...
        <div x-show="showImportModal" 
            ...
            <div class="bg-white/90 dark:bg-gray-900/90 backdrop-blur-xl p-8 rounded-3xl shadow-2xl w-full max-w-md border border-white/20"
                ...
                <form action="{{ route('rooms.import') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                    @csrf
    ...
    <script>
        ...
        function confirmDelete(id) {
            Swal.fire({
                title: 'Hapus Data?',
                text: "Data ruangan ini tidak dapat dikembalikan!",
                icon: 'warning',
